...
export class EmailVerificationService {
  ...
  static async verifyEmail(token: string): Promise<{id:number,full_name:string}> {
    try {
  ...
  static async resendVerificationEmail(email: string): Promise<{id:number,full_name:string}> {
    try {
      // Find user by email
      const user = await prisma.users.findUnique({
        where: { email },
   ...
   */
  static async sendVerificationEmailWithPassword(userId: number, tempPassword: string): Promise<void> {
    try {
      // Find user
      const user = await prisma.users.findUnique({
        where: { id: userId },
      ...
      await sendEmail({
        to: user.email,
        subject: 'Welcome to AcadeMe - Verify Your Email and Access Your Account',
        html: htmlContent
      });
    } catch (error) {
  ...
  private static generateEmailTemplate(data: EmailTemplateData): string {
    try {
      ...
      const templatePath = path.resolve(__dirname, '../../templates/emails/verification-email.hbs');
   ...
   * @returns HTML content as string
   */
  private static generateVerificationOnlyTemplate(data: EmailTemplateData): string {
    try {
      // Read the template file
      const templatePath = path.resolve(__dirname, '../../templates/emails/verification-only-email.hbs');
      const template = fs.readFileSync(templatePath, 'utf8');
      ...
      throw new AppError(500, 'Failed to generate verification-only email template');
    }
  }
} 